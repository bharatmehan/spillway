...
The simplest gauge, and often the one that actually binds. A provider's tokens
per minute limit can be comfortably clear while every request is queueing behind
other requests, because what is scarce is not the quota, it is the capacity
...
class Concurrency:
    ...
    One unit is taken at admission and given back at settlement, whatever the
    request turned out to cost. Size does not matter here: one request occupies
    one slot whether it generates ten tokens or ten thousand.

    Args:
        name: What this limit is called, in configuration and in an
            explanation.
    ...
    # The cost is ignored on purpose, and the signature keeps it because the
    # dimension protocol passes it to every axis.
    ...
    # Both costs are ignored on purpose. See the docstring: a slot has no
    # difference to reconcile, so there is nothing in them to read.
    def settle(
        self,
        ...
        occupied for the duration of the call and now it is not, so what comes
        back is exactly what was taken, however wrong the cost estimate was.
        """
