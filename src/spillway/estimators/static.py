...
A classifier answering with one of five labels, or an extractor filling a fixed
schema, produces nearly the same number of tokens every time, and a history of a
thousand identical samples would say nothing new.
"""
...
class StaticEstimator:
    ...
    Input is still counted per request. Only the output prediction is fixed,
    that being the part which cannot be counted.
